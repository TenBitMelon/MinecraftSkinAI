...
def load_image(filename, input_folder, output_folder):
    # if expect_multiple_images is true then split the filename by the first underscore and use the first part as the filename
        
    input_path = os.path.join(input_folder, filename)

    if expect_multiple_images:
...
with tf.device("/GPU:0"), tf.distribute.OneDeviceStrategy("/GPU:0").scope():    
    ...
    models = [
        ...
        # },
        # { # model used simplified images no noise
    ...
    for model_config in models:
        ...
        history = model.fit(dataset, epochs=7, steps_per_epoch=num_loaded_images // batch_size, callbacks=callbacks)
